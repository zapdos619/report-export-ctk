# main_app.py - Part 1: Main Application Setup
# CustomTkinter-based Salesforce Report Exporter
...
class SalesforceExporterApp(ctk.CTk):
    ...
    def _setup_reports_tab(self):
        ...
        placeholder_label.grid(row=0, column=0, pady=20)
    ...
    # main_app.py - Part 2: Footer, Login Handler, and Basic Operations
# Add these methods to the SalesforceExporterApp class

    def _create_footer(self):
        """Create footer with export controls"""
        footer_frame = ctk.CTkFrame(self, height=140)
        footer_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=(0, 10))
        footer_frame.grid_propagate(False)
        footer_frame.grid_columnconfigure(0, weight=1)
        
        # Output location section
        output_frame = ctk.CTkFrame(footer_frame)
        output_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=(10, 5))
        ...
        output_label.grid(row=0, column=0, padx=(10, 10), pady=10)
        ...
        self.output_path_label.grid(row=0, column=1, sticky="ew", padx=(0, 10), pady=10)
        ...
        self.progress_bar.grid(row=1, column=0, sticky="ew", padx=20, pady=(5, 5))
        ...
        self.progress_label.grid(row=2, column=0, sticky="ew", padx=20, pady=(0, 5))
        ...
        self.export_button.grid(row=3, column=0, sticky="ew", padx=20, pady=(5, 10))
    ...
    def _refresh_folders(self):
        ...
        self.folder_info_label.configure(text="Loading folders...")
    ...
    def _on_folders_error(self, error: str):
        ...
        self.folder_info_label.configure(text=f"Error: {error}")
        self.refresh_folders_btn.configure(state="normal")
    ...
    def _populate_folder_dropdown(self, folders: List[Dict], search_term: str = ""):
        ...
        if not folders:
            ...
            self.folder_info_label.configure(text="No report folders found")
            return
        ...
        if not folders and search_term:
            ...
            self.folder_info_label.configure(text=f"No matches for '{search_term}'")
            return
        ...
        # Update info
        if search_term:
            self.folder_info_label.configure(text=f"Found {len(folders)} folders matching '{search_term}'")
        else:
            self.folder_info_label.configure(text=f"Found {len(folders)} folders available")
    ...
    def _on_folder_selected(self, choice):
        ...
        if choice and hasattr(self, 'folder_map'):
            ...
            if folder_id == "ALL":
                self.folder_info_label.configure(text="Will export all reports from all folders")
            elif folder_id:
                folder_name = choice.split(" ", 1)[1] if " " in choice else choice
                self.folder_info_label.configure(text=f"Selected: {folder_name}")
    ...
    # main_app.py - Part 3: Report Selection Operations
# Add these methods to the SalesforceExporterApp class
    ...
    def _on_reports_error(self, error: str):
        ...
        messagebox.showerror("Error", f"Failed to load reports: {error}")
    ...
    # main_app.py - Part 4: Export Operations and Entry Point
# Add these final methods to complete the SalesforceExporterApp class
